...
outdir="/Shares/down/heatshock/analysis/ATAC_peaks_diff_exp/deseq2/peakandgene_controlsfwithgenes_ignoregenotype/"
...
ddsboth <- DESeqDataSetFromMatrix(countData =  countdat, colData = metadata, design = ~genotype + treatment) #use this for all samples
...
respeaks=results(dds_controlgenes, contrast=c(contrastcol,sample1,sample2))
summary(respeaks, alpha=cutoff)
DESeq2::plotMA(respeaks, main="D21 heat shock peaks", alpha=cutoff)
write.csv(as.data.frame(respeaks), file=paste(outdir,"res.csv",sep=""))

resSig_peak <- subset(respeaks, padj < cutoff)
resSig_peak <- as.data.frame(resSig_peak) %>% rownames_to_column() %>% filter(str_detect(rowname, "chr")) %>% column_to_rownames()
